...
import { ArrowDown, ArrowUpRight, Gift, ShieldCheck, Smartphone } from "lucide-react";
...
export default function FocusInfo() {
  ...
  return <div className="min-h-screen">
    ...
    <main className={`container ${employer ? "max-w-6xl" : "max-w-4xl"} py-14 md:py-20`}>
      ...
      {employee ? <ShareCompany /> : employer ? <section className="rounded-3xl bg-primary/5 border border-primary/10 p-6 sm:p-8">
      ...
      </section> : <div className="flex flex-wrap gap-3"><Button asChild><Link to="/demo/manager">{t("demo")}</Link></Button><Button asChild variant="outline"><a href="mailto:joel@teamfokus.app">{t("contactJoel")}</a></Button></div>}
    </main>
    <Footer />
  </div>;
}
