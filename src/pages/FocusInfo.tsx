...
export default function FocusInfo() {
  ...
  return <div className="min-h-screen"><Seo title={`TeamFokus · ${t(title)}`} description={t(body)} path={pathname} /><LandingHeader onDemo={() => navigate(employee ? "/demo/employee" : "/demo/manager")} /><main className="container max-w-4xl py-14 md:py-20"><Reveal><h1 className="text-4xl sm:text-5xl font-semibold tracking-tight max-w-2xl">{t(title)}</h1><p className="text-lg text-muted-foreground max-w-2xl leading-relaxed mt-5">{t(body)}</p></Reveal>{employer ? <EmployerBenefits /> : <div className="grid md:grid-cols-3 gap-4 my-9">{items.map((item) => <Reveal key={item.title}><DepthCard className="p-6"><item.icon className="h-6 w-6 text-primary mb-5" /><h2 className="font-semibold">{t(item.title)}</h2><p className="text-sm text-muted-foreground leading-relaxed mt-3">{t(item.body)}</p></DepthCard></Reveal>)}</div>}{employee ? <ShareCompany /> : <div className="flex flex-wrap gap-3"><Button asChild><Link to="/demo/manager">{t("demo")}</Link></Button><Button asChild variant="outline"><a href="mailto:joel@teamfokus.app">{t("contactJoel")}</a></Button></div>}</main><Footer /></div>;
}
