...
export default function CompanySuggestion() {
  ...
  return <div className="min-h-screen flex flex-col"><Seo title={`TeamFokus · ${t("suggestCompany")}`} description={t("companyBody")} path="/unternehmen-vorschlagen" /><LandingHeader onDemo={() => navigate("/demo/employee")} /><main className="container max-w-xl py-14 flex-1"><h1 className="text-3xl sm:text-4xl font-semibold tracking-tight">{t("companyTitle")}</h1><p className="text-muted-foreground leading-relaxed mt-4 mb-8">{t("companyBody")}</p>{sent ? <div className="surface-card p-7 flex gap-3 items-center" role="status"><CheckCircle2 className="w-6 h-6 text-primary shrink-0" /><p>{t("sent")}</p></div> : <form onSubmit={submit} className="surface-card p-6 space-y-5">
