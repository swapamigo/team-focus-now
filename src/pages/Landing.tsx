...
import { ArrowRight, Gift, Leaf, LockKeyhole, Sparkles, Sun, Trophy } from "lucide-react";
  ...
  return <div className="min-h-screen bg-background">
    ...
    <main>
      ...
      <section id="rewards" className="container max-w-5xl pb-14 md:pb-20 scroll-mt-24">
        ...
        <div className="grid sm:grid-cols-3 gap-4 mt-7">{[{ icon: Gift, key: "giftExample" }, { icon: Leaf, key: "massageExample" }, { icon: Sun, key: "timeExample" }].map(({ icon: Icon, key }) => <article key={key} className="rounded-2xl border bg-card p-6"><div className="h-24 rounded-xl bg-gradient-to-br from-primary/5 to-accent/10 flex items-center justify-center mb-5"><Icon className="h-9 w-9 text-primary" strokeWidth={1.5} /></div><h3 className="font-medium leading-snug">{t(key as "giftExample" | "massageExample" | "timeExample")}</h3></article>)}</div>
      </section>
