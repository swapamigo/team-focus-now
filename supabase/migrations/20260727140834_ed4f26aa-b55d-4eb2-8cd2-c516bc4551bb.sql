...
CREATE POLICY daily_team_summaries_manager_insert
...
WITH CHECK (
  ...
  AND member_count >= COALESCE(
    (SELECT c.min_team_k FROM public.companies c WHERE c.id = daily_team_summaries.company_id),
    5
  )
);

CREATE POLICY daily_team_summaries_manager_update
ON public.daily_team_summaries
FOR UPDATE
TO authenticated
USING (
...
CREATE POLICY daily_team_summaries_manager_delete
ON public.daily_team_summaries
FOR DELETE
TO authenticated
USING (
